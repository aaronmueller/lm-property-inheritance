...
#    no_token = tokenizer.convert_ids_to_tokens(tokenizer.encode(example_prompt + " " + "No")[-1])
    ...
    for index, row in dfshuffled.iterrows():
        ...
        if flip_pair:
            ...
            samples.append(
                    {"conclusion_": row['premise'],
                     "premise_": row['conclusion'],
            ...
            samples.append(
                    {"conclusion_": row['conclusion'],
                     "premise_": row['premise'],
...
def icl_accuracy(dataloader, model, tokenizer, yes_label, no_label):
    ...
    with torch.no_grad():
        for step, inputs in enumerate(tqdm(dataloader)):
            ...
            logprobs_yes_id = logprobs[:, Yes_id].tolist()
            logprobs_no_id = logprobs[:, No_id].tolist()

            all_logprobs_no.extend(logprobs_no_id)
            all_logprobs_yes.extend(logprobs_yes_id)
    ...
    return current_acc, gold, preds, yes_over_no#, all_logprobs_yes, all_logprobs_no
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="A simple calculator that works with integers and floats.")
    ...
    if compute_all_combinations:
        ...
        for ii,layer in enumerate(layers):
            for jj, relative_pos in enumerate(relpos):
                ...
                intervenable, eval_metrics = train_boundless_das(layer, Yes_id, No_id, train_dataloader, test_dataloader, detail_string, verbose=False, epochs=epochs, evaluateit=eval_during_train, outputdirname=output_dir_name, filter_name = train_filter, cuda_gpu=cuda_gpu, control=control)
